...
for N in ntiles:
    with open("{}/{}.txt".format(summary_path, N), "w") as dest:
        print("SUMMARY OF EMPIRICAL LONGEST PATH DISTRIBUTION.", file=dest) 
        print("MATRIX SIZE: {}x{}".format(N,N), file=dest)
        ...
        data = {}
        for dist in dists:
            with open('data/{}/{}.dill'.format(dist, N), 'rb') as file:
                D = dill.load(file)
            data[dist] = D
            print("\n\n\n---------------------------------", file=dest)
...
#             # axs[i, j].axvline(np.mean(D), color='k', linestyle='solid', linewidth=1)
#             if j == 0:
#                 axs[i, j].set_ylabel("N = {}".format(N), fontsize=6)
#             if N == 50:
