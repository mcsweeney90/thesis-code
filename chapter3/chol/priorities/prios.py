...
# -*- coding: utf-8 -*-
"""
Critical path approximation methods for priorities in HEFT.
"""
...
nprocessors = 20
Vs = [0.2, 1.0]
ccrs = [0.01, 0.1, 1.0, 10.0] # TODO: what to do about CCR = 10?

avgs = ["M", "MD", "B", "SB", "W", "SW", "HM", "SHM", "GM", "SGM", "R", "D", "NC", "SD"]
bounds = ["LB", "UB"]
# mc = ["{}{}-{}".format(a, b, c) for a, b, c in product(["A", "H"], [10, 1000], ["M", "U", "C"])]
...
clean = {0.01 : "001", 0.1 : "01", 1.0 : "1", 10.0 : "10", 0.2 : "02"}
...
def plot_mpd(data, name, ylabel=True):
    ...
    fig.tight_layout()    
    plt.savefig('{}/prio_chol_mpd_{}'.format(mpd_plot_path, name), bbox_inches='tight') 
    plt.close(fig)       

def plot_bests(data, name, ylabel=True):
    """Plot MPD and best instances."""
    ...
    # Best instances.
    wins = list( 100*((abs(bests - data[method])).values == 0.0).sum()/data.shape[0] for method in all_h)    
    fig, ax = plt.subplots(dpi=400)
    rects = ax.bar(x, wins, color=colors)    
    plt.minorticks_on()
    plt.grid(True, linestyle='-', axis='y', which='major')
    plt.grid(True, linestyle=':', axis='y', which='minor')
    plt.grid(False, axis='x')
    ...
    ax.tick_params(axis='x', which='minor', bottom=False)       
    fig.tight_layout()    
    plt.savefig('{}/prio_chol_bests_{}'.format(best_plot_path, name), bbox_inches='tight') 
    plt.close(fig)     
