...
def summarize(data, save_dest):
    with open(save_dest, "w") as dest:
        ...
        for method in all_h:
            ...
            # if method in mc: # TODO: check this...
            #     time = data["{}-REAL TIME".format(method[:-2])] + data["{}-LONG TIME".format(method[:-2])]
            #     if method[-1] == 'C':
            #         time += data["{}-CRT TIME".format(method[:-2])]
            #     incs = 100*(time - data["HEFT PRIO TIME"])/data["HEFT PRIO TIME"]
            #     print("\nTIME INCREASE", file=dest)
            #     print("----------------------------", file=dest)
            #     print("MEAN = {}%".format(incs.mean()), file=dest)
            #     print("BEST = {}%".format(incs.min()), file=dest)
            #     print("WORST = {}%".format(incs.max()), file=dest)
